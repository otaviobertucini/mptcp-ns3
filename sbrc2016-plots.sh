...
PlotCWND(){
  ...
  if [ ${#DATAFILES[@]} != 0 ]; then
    ...
    do
      ...
      gTITLE="${queueName[$fe]} Janela de Congestionamento"
      xLABEL="Atraso no caminho A (ms)"
      yLABEL="Tamanho médio (pacotes)"
      yRANGE="[0:40]"
      xRANGE="[-0.5:2.5]"
      xTICS="('1' 0, '10' 1, '100' 2)"
      yTICS="5"
...
PlotMultiCWND(){
  ...
  DATAFILES=(`find ./plots -maxdepth 1 -iname "*$CC*CWND*"`)
  if [ ${#DATAFILES[@]} != 0 ]; then
      gTITLE="Janela de Congestionamento"
      xLABEL="Atraso no caminho A (ms)"
      yLABEL="Tamanho médio (pacotes)"
      yRANGE="[0:40]"
      xRANGE="[-0.5:2.5]"
...
PlotLGHTAll(){
  CC=$1
  echo -e "PLOT LENGHT QUEUE GRAPHS"
  DATAFILES=(`find ./plots -maxdepth 1 -iname "*LGHT*"`)
  if [ ${#DATAFILES[@]} != 0 ]; then
      gTITLE="${queueName[$fe]} Queue Size"
      xLABEL="Atraso no caminho A (ms)"
      yLABEL="Tamanho médio (pacotes)"
...
PlotSJN(){
  ...
  if [ ${#DATAFILES[@]} != 0 ]; then
      ...
      xLABEL="Atraso no caminho A (ms)"
      yLABEL="Tempo médio de permanência (ms)"
      yRANGE="[0:0.1]"
      xRANGE="[-0.5:2.5]"
      xTICS="('1' 0, '10' 1, '100' 2)"
      yTICS="0.01"
...
PlotSJNAll(){
  CC=$1
  echo -e "PLOT LENGHT QUEUE GRAPHS"
  DATAFILES=(`find ./plots -maxdepth 1 -iname "*SJN*"`)
  if [ ${#DATAFILES[@]} != 0 ]; then
      gTITLE="${queueName[$fe]} Sojourn Time"
      xLABEL="Atraso no caminho A (ms)"
      yLABEL="Tempo médio de permanência (ms)"
...
plotBox(){
  ...
  echo "
          ...
          set style line 1 lc rgb '#FF0000' lt 1 dashtype 1 lw 2 pt 2
          set style line 2 lc rgb '#0000FF' lt 1 dashtype 1 lw 2 pt 12
          ...
          plot '${PLOTFILE}' using 0:1:2:3 t 'WiFi' with errorbars ls 1, \
...
plotMonoBox(){
  ...
  echo "
          ...
          #cria estilo para a caixa e os pontos
          set style line 1 lc rgb '#FF0000' lt 1 dashtype 1 lw 1 pt 2
          set style line 2 lc rgb '#0000FF' lt 1 dashtype 1 lw 2 pt 12
          ...
          set yrange ${yRANGE}
          set xrange ${xRANGE}
          set xtics nomirror scale 0 font 'Helvetica,14'
          #cria a grade
          set grid ytics ls 4
          set grid xtics ls 4
...
plotMultiPlotBox(){
  ...
  gnucmd=`echo "
          ...
          #cria estilo para a caixa e os pontos
          set style line 1 lc rgb '#FF0000' lt 1 dashtype 1 lw 1 pt 2
          set style line 2 lc rgb '#0000FF' lt 1 dashtype 1 lw 1 pt 12
          set style line 3 lc rgb '#B5B5B5' lt 3 dashtype 2 lw 0.5
          set style line 4 lc rgb '#000000' lt 1 dashtype '-' lw 0.3
...
plotBars(){
  ...
  gnucmd=`echo "
          ...
          #titulos
          #set title '$gTITLE'
          set xlabel '${xLABEL}' font 'Helvetica,22'
...
plotBars2(){
  ...
  gnucmd=`echo "
          ...
          #posição da legenda
          set key left top samplen 4 font 'Helvetica,18' reverse Left
  ...
  do
    sty=$(($fe + 1))
    gnucmd+="'${DATAFILES[$fe]}' using 1:2:3 t '${congestionControl[$fe]}' ls $sty, "
  done
  #echo -e "$gnucmd"
  echo -e "$gnucmd" | gnuplot
...
Plot(){
  ...
  do
      ...
      PlotLGHTAll ${congestionControl[$congestion]}
      PlotSJN ${congestionControl[$congestion]}
      PlotSJNAll ${congestionControl[$congestion]}
