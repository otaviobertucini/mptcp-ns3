...
#define DEFAULT_CODEL_LIMIT 1000
    ...
    TypeId CoDelQueue::GetTypeId (void)
    {
        ...
        .AddTraceSource("QueueStackSize",
                        "Queue or Stack size",
                        MakeTraceSourceAccessor(&CoDelQueue::trace_size),
                        "ns3::TracedValue::Uint32Callback")
        ;
    ...
    CoDelQueue::DoDequeue (void)
    {
            ...
            if (!drop)
            {
                ...
                if (codel_time_after_eq(now, m_drop_next))
                {
                    ...
                    while (m_dropping && codel_time_after_eq(now, m_drop_next)) {
                        /*
                         std::string filename = "bfs_codel_drop_packets.tracer.tr";
                         Ptr<OutputStreamWrapper> stream = Create<OutputStreamWrapper>(filename.c_str(), std::ios::app);
                         Ptr<Packet> pp = p->Copy();
                         EthernetHeader ethHeader;
                         //Ipv4Header ipv4;
                         //pp->RemoveHeader (ipv4Header);
                         pp->PeekHeader(ethHeader);
                         *stream->GetStream () << ethHeader.GetSource() << " " << ethHeader.GetDestination() << " " << (double)Simulator::Now().GetSeconds() << std::endl;
                         */
        ...
        ++m_states;
        //if(drop)
        //std::cout << "Time:" << now/NSEC_PER_USEC << " DNext:" << m_drop_next/NSEC_PER_USEC <<" m_count:" << m_count  << " Size:" << m_packets.size () << '\n';
        return p;
    }
