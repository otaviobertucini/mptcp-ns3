...
namespace ns3
{
    ...
    MpTcpSocketBase::ProcessSynSent(uint8_t sFlowIdx, Ptr<Packet> packet, const TcpHeader& mptcpHeader)
    {
        ...
        { // Handshake completed for sender... Send final ACK
            ...
            NS_LOG_INFO("ESTABLISHED BETWEEN: " << sFlow->sAddr << ":" << sFlow->dAddr);
    ...
    MpTcpSocketBase::InitiateSubflows()
    {
        ...
        for (uint32_t i = 0; i < localAddrs.size(); i++){
            //NS_LOG_UNCOND("\n -> local " << localAddrs[i]->ipv4Addr << " m_local " << m_localAddress);

            for (uint32_t j = 0; j < remoteAddrs.size(); j++)
            {
                uint8_t addrID = localAddrs[i]->addrID;
                Ipv4Address local = localAddrs[i]->ipv4Addr;
                Ipv4Address remote = remoteAddrs[j]->ipv4Addr;

                //NS_LOG_UNCOND("\t-> remote" << remote<<" m_remote:" << m_remoteAddress);
                ...
                if (((local == m_localAddress) || (remote == m_remoteAddress && remoteAddrs.size() > 1)) || (!IsThereRoute(local, remote)))
                {
                    //NS_LOG_UNCOND("----- SKIP -------");
                    NS_LOG_INFO("InitiateSubflows -> Skip subflow which is already established or has not a route (" << local << " -> " << remote<<")" << IsThereRoute(local, remote));
                    continue;
                }
    ...
    MpTcpSocketBase::GenerateCwndTracer()
    {
        ...
        cwndTracerGraph.AppendExtra("set terminal postscript eps enhanced color solid font 'Helvetica,22'\n"
                                    "set output \'| epstopdf --filter > trace-cwd.pdf\'"
        ...
        sstGraph.AppendExtra(
                             "set terminal postscript eps enhanced color solid font 'Helvetica,15'\n"
                             "set output \"sst.eps\"\n"
                             "set xlabel\"Time (s)\" offset 0,-1\n"
    ...
    MpTcpSocketBase::GenerateRTT()
    {
        ...
        // RTO Plot
        Gnuplot rtoGraph;
        rtoGraph.AppendExtra("set terminal postscript eps enhanced color solid font 'Helvetica,15'\n"
                             "set output \"rto.eps\"\n"
                             "set xlabel \"Time (s)\" offset 0,-1\n"
    ...
    MpTcpSocketBase::GeneratePktCount()
    {
        ...
        pktCountGraph.AppendExtra("set terminal postscript eps enhanced color solid font 'Helvetica,15'\n"
                                  "set output \"pkt.eps\"\n"
        ...
        dataSetPKt.SetTitle("");
        dataSetPKt.SetExtra(" using 2:xtic(0) lc 62\n");

       // NS_LOG_UNCOND("::::::::::::::::::::::::::::::::::Flows size: " << subflows.size());

        for (uint16_t idx = 0; idx < subflows.size(); idx++)
        {
            Ptr<MpTcpSubFlow> sFlow = subflows[idx];
            dataSetPKt.Add(idx, sFlow->PktCount);
            //NS_LOG_UNCOND(":::::::::::::::::Flows Id: " << idx << " Pkt Ct:" << sFlow->PktCount);
        }
        pktCountGraph.AddDataset(dataSetPKt);
        gnu.AddPlot(pktCountGraph);
    }
    void
    MpTcpSocketBase::GenerateDataSets(void)
    {
        ...
        std::stringstream title;
        title << "mptcp-PKT-tracer.tr";
        Ptr<OutputStreamWrapper> pktstream = Create<OutputStreamWrapper>(title.str(), std::ios::app);
        ...
        for (uint16_t idx = 0; idx < subflows.size(); idx++)
        {
             ...
             *  RTT
             */
            std::stringstream titleRtt;
            titleRtt << "mptcp-RTT-" << idx << "-tracer.tr";
            stream = Create<OutputStreamWrapper>(titleRtt.str(), std::ios::app);
            vector<pair<double, double> >::iterator it = sFlow->rttTracer.begin();

            while (it != sFlow->rttTracer.end())
            {
                *stream->GetStream () << it->first << " " << it->second <<std::endl;
                it++;
            }
            /*
             *  CWND
             */
            std::stringstream titleCwd;
            titleCwd << "mptcp-CWND-" << idx << "-tracer.tr";
            stream = Create<OutputStreamWrapper>(titleCwd.str(), std::ios::app);
            vector<pair<double, uint32_t> >::iterator it2 = sFlow->cwndTracer.begin();
            while (it2 != sFlow->cwndTracer.end())
            {
                *stream->GetStream () << it2->first << " " << it2->second/sFlow->MSS <<std::endl;
                it2++;
            }
             ...
             *  Packet Count
             */
            *pktstream->GetStream () << idx << " " << sFlow->PktCount <<std::endl;
            /*
             *  TX QUEUE
             */
            std::stringstream titleTx;
            titleTx << "mptcp-TXQUEUE-" << idx << "-tracer.tr";
            stream = Create<OutputStreamWrapper>(titleTx.str(), std::ios::app);
            vector<pair<double, double> >::iterator ittx = TxQueue.begin();

            while (ittx != TxQueue.end())
            {
                *stream->GetStream () << ittx->first << " " << ittx->second <<std::endl;
                ittx++;
            }


            *stream->GetStream () << "sFlow:" << sFlow->sAddr << "CC:" << PrintCC(AlgoCC) << "  sFlowSize:" << subflows.size() << " Capacity:" << LinkCapacity << "Kbps  RTT:" << RTT << " Bytes:" << totalBytes / 1000 << "Kb  Loss(" << lostRate << ")  MSS:" << segmentSize << "Bytes" <<std::endl;
    ...
    MpTcpSocketBase::Close(uint8_t sFlowIdx)
    {
        ...
        { // App close with pending data must wait until all data transmitted from socket buffer
            ...
            {
                m_closeOnEmpty = true;
                if (flowType.compare("Large") == 0 || flowType.compare("Short") == 0)
    ...
    {
        return (uint16_t) 49640;//era 49640
    }
    ...
    MpTcpSocketBase::AvailableWindow(uint8_t sFlowIdx)
    {
        ...
        else
        {
            ...
            //NS_LOG_UNCOND("RemoteRecvWindow: ("<< (int)sFlowIdx <<") -> " << remoteRecvWnd );
            return freeCWND;
        }
    }
    ...
    //{
        ...
        oss << "benevid_" << PrintCC(AlgoCC) << "_" << (uint32_t)Simulator::Now().GetSeconds() <<"_"<< (int)maxSubflows << "_" << GetTypeIdName() <<"_" << m_node->GetId() << "_" << flowId <<"_" << m_rGap << ".plt";
        ...
        gnu.GenerateOutput (std::cout);
        //  oss.str("");
        //  outfile.close();
        */
        /*std::string fileName = "_PLOT.";
        ...
        //uint32_t fileTime = (int)time(NULL);

        std::stringstream sstm;
        sstm << fileName << buf << fileExt;

        //std::string plotFileName         = "_PLOT_"++".plt";
        std::ofstream plotFile (sstm.str());

        // Write the plot file.
        gnu.GenerateOutput (plotFile);
