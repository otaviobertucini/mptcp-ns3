...
TcpHeader::Print(std::ostream &os) const
{
  ...
  for (uint32_t j = 0; j < mp_option.size(); j++)
    {
      os << " {";
      TcpOptions *opt = mp_option[j];
...
{
   if((m_length*4)< (CalculateHeaderLength () * 4))
  ...
  while (!i.IsEnd() && hlen > 0)
    {
      ...
      mp_option.insert(mp_option.end(), opt);
...
TcpHeader::~TcpHeader()
{
  if (original == false)
    return;
  //NS_LOG_FUNCTION_NOARGS();
  for (uint32_t i = 0; i < mp_option.size(); i++)
    {
      if (mp_option[i] != 0)
        switch (mp_option[i]->optName)
          {
        case OPT_MPC:
          delete (OptMultipathCapable*) mp_option[i];
          break;
        case OPT_JOIN:
          delete (OptJoinConnection*) mp_option[i];
          break;
        case OPT_ADDR:
          delete (OptAddAddress*) mp_option[i];
          break;
        case OPT_DSN:
          delete (OptDataSeqMapping*) mp_option[i];
          break;
        default:
          break;
          }
