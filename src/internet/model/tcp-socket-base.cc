...
TcpSocketBase::TcpSocketBase (const TcpSocketBase& sock)
  : TcpSocket (sock),
    ...
    m_ssThresh (sock.m_ssThresh), //BFS
    m_cWnd (sock.m_cWnd)
...
TcpSocketBase::SetupCallback (void)
{
  ...
  if (m_endPoint6 != 0) //MPTCP comment this
    {
...
TcpSocketBase::ProcessSynSent (Ptr<Packet> packet, const TcpHeader& tcpHeader)
{
  ...
  if (tcpflags == 0)
    ...
    { // Handshake completed
      ...
      SendEmptyPacket (TcpHeader::ACK);
      fLowStartTime = Simulator::Now().GetSeconds();//MPTCP
...
bool
TcpSocketBase::IsCorrupt(Ptr<Packet> p) //MPTCP
{
...
TcpSocketBase::Retransmit ()
{
  ...
  m_rtt->IncreaseMultiplier(); // Double the timeout value for next retx timer //MPTCP
  m_dupAckCount = 0;
  ...
  //Congestion Avoidence
    {
      ...
      dataSet.SetTitle("Congestion Avoidance");
